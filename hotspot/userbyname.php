...
// share WhatsApp
  if(substr($getvalid,-1) == "w"){
  $getvalid = "".substr($getvalid,0,-1)." MINGGU";}
else if(substr($getvalid,-1) == "d"){
  $getvalid = "".substr($getvalid,0,-1)." HARI";}
else if(substr($getvalid,-1) == "h"){
  $getvalid = "".substr($getvalid,0,-1)." Jam";}

  if ($getvalid != "") {
    $wavalid = "Masa Aktif"." : *" . $getvalid . "* %0A";
  } else {
    $wavalid = "";
  }

if(substr($utimelimit,-1) == "d" & strlen($utimelimit) >3){
  $utimelimit = "".((substr($utimelimit,0,-1)*7) +  substr($utimelimit, 2,1))." HARI";
}else if(substr($utimelimit,-1) == "d"){
  $utimelimit = "".substr($utimelimit,0,-1)." HARI";
}else if(substr($utimelimit,-1) == "h"){
  $utimelimit = "".substr($utimelimit,0,-1)."Jam";
}else if(substr($utimelimit,-1) == "w"){
  $utimelimit = "".(substr($utimelimit,0,-1)*7)." HARI";
}


if ($utimelimit != "") {
  $watlimit = "Durasi"." : *" . $utimelimit . "* %0A";
} else {
  $watlimit = "";
  $bMB = "";
}
  ...
  if ($udatalimit != "") {
    $wadlimit = "Kuota"." : *" . $udatalimit . "" . $MG . "* %0A";
    $bMG = $MG;
  } else {
    $wadlimit = "Kuota"." : *"."Unlimited"."* %0A";
  }
...
if ($currency == in_array($currency, $cekindo['indo'])) {
      $getprice = "Harga"." : *" . $currency . " " . number_format($getprice, 0, ",", ".") . "* %0A";
      $btprice = $getprice;
    } else {
